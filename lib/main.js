...
function iterator( alpha, beta, options ) {
	...
	if ( !isPositiveNumber( alpha ) ) {
		throw new TypeError( format( '0pj71,NQ', alpha ) );
	}
	if ( !isPositiveNumber( beta ) ) {
		throw new TypeError( format( '0pj72,Go', beta ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0pj2V,FD', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0pj2t,G9', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
