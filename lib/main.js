...
function iterator( alpha, beta, options ) {
	...
	if ( !isPositiveNumber( alpha ) ) {
		throw new TypeError( format( '0Pl7P', alpha ) );
	}
	if ( !isPositiveNumber( beta ) ) {
		throw new TypeError( format( '0Pl7T', beta ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Pl2h', options ) );
		}
		opts = copy( options, 1 );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Pl35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
